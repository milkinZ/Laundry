    ...
    $id_user = $_SESSION['id'];
            ...
            if(empty($tgl)){
                ...
                for ($i=0; $i < count($qty); $i++) {
                    var_dump($qty, $id_paket, $id_transaksi);
                    $insert=mysqli_query($conn,"insert into detil_transaksi (id_transaksi, id_paket, qty) value ($id_transaksi, ".$id_paket[$i].", ".$qty[$i].")");
                    echo mysqli_error($conn);    
                }
                if($insert){
                    echo"<script>alert('Sukses menambahkan');location.href='transaksi.php';</script>";
                }else{
                    echo"<script>alert('gagal menambahkan ')location.href='tampil_transaksi.php';</script>";
                }
            }
        }